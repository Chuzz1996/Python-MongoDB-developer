...
def get_db():
    ...
    if db is None:
        ...
        """
        db = g._database = MongoClient(MFLIX_DB_URI,maxPoolSize=50,wtimeout=2500
...
def get_movies_by_country(countries):
    ...
    try:
        ...
        print("Se eesta ejecutando")
        data = "db.movies.insertMany("+str(list(db.movies.find({})))+");"
        f = open("movies.js", "w+")
        f.write(data)
        f.close()
        print("Se ejecuto")
...
def most_active_commenters():
    ...
    limit_state = {"$limit":20}
    pipeline = [group_state,sort_stage,limit_state]
